...
def get_report(analytics,date,startdate):
  ...
  headers=""
  list_dimension=[{'name': 'ga:language'},{'name':'ga:date'}]
...
def print_response(response,date,headers,startdate):
  ...
  datelist=date.split('-')
  address="googleanalytics/landing/languagereport/"
  file_name=address+"languagereport"+date+".csv"
  importtos3.write_to_s3(stringfordata,'paragon-datalake',file_name)
  
  '''
  text_file = open("languagereport.csv", "w")
