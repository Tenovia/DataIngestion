...
def get_report(analytics,date,startdate):
  ...
  headers=""
  list_dimension=[{'name': 'ga:city'},{'name':'ga:date'}]
...
def print_response(response,date,headers,startdate):
  ...
  datelist=date.split('-')
  #print date
  address="googleanalytics/landing/cityreport/"
  file_name=address+"cityreport"+date+".csv"
  importtos3.write_to_s3(stringfordata,'paragon-datalake',file_name)
  
  '''
  text_file = open("cityreport.csv", "w")
