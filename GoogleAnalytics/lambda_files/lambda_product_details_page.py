...
def get_report(analytics,date,startdate):
  ...
  headers=""
  list_dimension=[{'name':'ga:productName'},{'name': 'ga:productSku'},{'name':'ga:date'}]

  list_metrics=[{'expression':'ga:itemRevenue'},{'expression':'ga:uniquePurchases'},{'expression': 'ga:itemQuantity'},{'expression':'ga:revenuePerItem'},{'expression':'ga:itemsPerPurchase'},{'expression':'ga:productRefundAmount'},{'expression':'ga:cartToDetailRate'},{'expression':'ga:buyToDetailRate'},{'expression':'ga:productListCTR'}]
...
def print_response(response,date,headers,startdate):
  ...
  address="googleanalytics/landing/product_details/"
  file_name=address+"product_details_page"+date+".csv"
  importtos3.write_to_s3(stringfordata,'paragon-datalake',file_name)
  
  '''
  text_file = open("product_details.csv", "w")
