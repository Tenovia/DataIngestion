...
def publisher_platform():
    ...
    fields1=['account_name','campaign_name', 'campaign_id','reach','cost_per_action_type:landing_page_view', 'unique_actions:page_engagement', 'cost_per_unique_action_type:page_engagement', 'spend', 'cost_per_unique_action_type:landing_page_view', 'cost_per_unique_outbound_click:outbound_click', 'action:page_engagement', 'unique_actions:landing_page_view', 'cpp', 'cpm', 'cost_per_action_type:link_click', 'instant_experience_clicks_to_start', 'unique_actions:post_engagement', 'publisher_platform', 'cost_per_action_type:page_engagement', 'inline_post_engagement', 'outbound_clicks:outbound_click', 'action:post_engagement', 'unique_outbound_clicks_ctr:outbound_click', 'unique_clicks', 'cpc', 'cost_per_unique_click', 'impressions', 'cost_per_unique_action_type:link_click', 'unique_outbound_clicks:outbound_click', 'website_ctr:link_click', 'action:link_click', 'cost_per_action_type:post_engagement', 'unique_actions:link_click', 'date_start', 'inline_link_click_ctr', 'instant_experience_clicks_to_open', 'objective', 'platform_position', 'frequency', 'cost_per_inline_post_engagement', 'inline_link_clicks', 'cost_per_inline_link_click', 'action:landing_page_view', 'date_stop', 'unique_inline_link_clicks', 'unique_ctr', 'ctr', 'cost_per_unique_action_type:post_engagement', 'instant_experience_outbound_clicks', 'unique_inline_link_click_ctr', 'cost_per_outbound_click:outbound_click', 'clicks', 'cost_per_unique_inline_link_click', 'unique_link_clicks_ctr', 'outbound_clicks_ctr:outbound_click']   
    ...
    for x in range(1,8):
        ...
        year=arr[0]
        month=arr[1]
        params = {'time_range': {'since': date1, 'until': date1},'breakdowns':['publisher_platform','platform_position']}
        fields = [
            'account_name',
            'campaign_name',
            'campaign_id',
        ...
        for i in range(len(campaignids)):
            ...
            try:
                ...
                list_d.append(dic_camp)
                    ...
                    continue
        if list_d:
            filename1="FacebookAds/landing_publisher_platform/"+"publisher_platform_"+date1+".csv"
            filename2="FacebookAds/processing_publisher_platform/csv/"+year+"/"+month+"/"+"publisher_platform_"+date1+".csv"
