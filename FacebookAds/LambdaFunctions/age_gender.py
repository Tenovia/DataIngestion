...
def age_gender():
    ...
    fields1=['account_name','campaign_name', 'campaign_id', 'reach','unique_actions:post_reaction', 'unique_actions:post', 'action:post', 'cost_per_action_type:post','cost_per_action_type:post_reaction', 'action:post_reaction','cost_per_unique_action_type:post_engagement',  'outbound_clicks:outbound_click', 'age', 'gender', 'cost_per_action_type:comment', 'unique_actions:comment', 'action:comment', 'action:photo_view', 'unique_actions:photo_view', 'estimated_ad_recall_rate', 'cost_per_action_type:photo_view', 'estimated_ad_recallers', 'cost_per_estimated_ad_recallers','cost_per_unique_action_type:page_engagement', 'cost_per_outbound_click:outbound_click', 'cost_per_action_type:link_click', 'cost_per_action_type:post_engagement', 'inline_link_clicks', 'unique_actions:landing_page_view', 'inline_link_click_ctr', 'cost_per_action_type:page_engagement', 'frequency', 'outbound_clicks_ctr:outbound_click', 'unique_outbound_clicks:outbound_click', 'cost_per_unique_action_type:link_click', 'spend',  'unique_actions:page_engagement', 'unique_actions:link_click', 'unique_outbound_clicks_ctr:outbound_click',  'unique_ctr', 'cpm', 'unique_clicks', 'cpc', 'cost_per_unique_click', 'website_ctr:link_click', 'cost_per_unique_action_type:landing_page_view', 'action:post_engagement', 'clicks', 'action:link_click', 'objective', 'cpp','instant_experience_clicks_to_open', 'unique_link_clicks_ctr', 'instant_experience_clicks_to_start', 'impressions', 'unique_inline_link_click_ctr', 'cost_per_inline_post_engagement', 'inline_post_engagement', 'cost_per_action_type:landing_page_view', 'unique_inline_link_clicks', 'date_start', 'cost_per_unique_outbound_click:outbound_click', 'cost_per_inline_link_click', 'unique_actions:post_engagement', 'action:page_engagement', 'instant_experience_outbound_clicks', 'ctr', 'action:landing_page_view', 'cost_per_unique_inline_link_click', 'date_stop']
    ...
    for x in range(1,8):
        ...
        year=arr[0]
        month=arr[1]
        params = {'time_range': {'since': date1, 'until': date1},'breakdowns':['age','gender']}
        fields = [
            'account_name',
            'campaign_name',
            'campaign_id',
        ...
        for i in range(len(campaignids)):
            ...
            try:
                ...
                list_d.append(dic_camp)
                    ...
                    continue
        if list_d:
            filename1="FacebookAds/landing_age_gender/"+"age_gender_"+date1+".csv"
            filename2="FacebookAds/processing_age_gender/csv/"+year+"/"+month+"/"+"age_gender_"+date1+".csv"
